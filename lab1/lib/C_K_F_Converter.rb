...
class Reader
	...
	def read()
		...
		if (@in_degree_type != "C" and @in_degree_type != "K" and @in_degree_type != "F")
		...
		if (@out_degree_type != "C" and @out_degree_type != "K" and @out_degree_type != "F")
		...
		case(@reader.in_degree_type())
			when("C")
				case(@reader.out_degree_type())
					when("C")
						return @reader.temperature(), @reader.out_degree_type()
					when("K")
						return (@reader.temperature() + 273), @reader.out_degree_type()
					when("F")
						return (@reader.temperature() * 1.8 + 32), @reader.out_degree_type()	
				end
			when("K")
				case(@reader.out_degree_type())
					when("C")
					...
					when("K")
						return @reader.temperature(), @reader.out_degree_type()
					when("F")
						return (@reader.temperature() * 1.8 - 459), @reader.out_degree_type()	
				end
			when("F")
				case(@reader.out_degree_type())
					when("C")
						return ((@reader.temperature() - 32) / 1.8), @reader.out_degree_type()
					when("K")
						return ((@reader.temperature() + 459) / 1.8), @reader.out_degree_type()
					when("F")
						return @reader.temperature(), @reader.out_degree_type()
				end			
		end
